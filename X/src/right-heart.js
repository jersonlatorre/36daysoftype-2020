	...
	draw() {
		...
		switch (this.state) {
			case HeartState.NONE: {
				...
				if (mouseIsPressed && dist(mouseX - 100, mouseY, this.position.x, this.position.y) < 150 && !LeftHeart.isDragging) {
			...
			case HeartState.DRAGGING: {
				RightHeart.isDragging = true
				this.position.x = mouseX
				this.position.y = mouseY
				let angle = atan2(
					mouseY - this.initialPosition.y - this.offset.x,
					mouseX - this.initialPosition.x - this.offset.y
				)
